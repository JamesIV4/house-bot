...
| Item | Known details | Status / questions |
|---|---|---|
| Mobile platform | LEGO tread base; 5.5 in tread section plus about 1 in rear extension, 6 in outside width | Approximate assembled footprint is 6.5 x 6 in; expect rebuilds and small geometry shifts |
| Drive arrangement | Two independently driven treads; forward, reverse, and both pivot combinations verified | Fit effective skid-steer width from motion because tread scrub makes geometric spacing misleading |
...
|---|---|---|
| Logitech HD Pro Webcam C920 | Windows DirectShow and Pi V4L2 expose MJPEG/H.264; Pi `/dev/video0` supplies native H.264 at 1280x720/30 FPS; fixed focus 0; measured `fx=940.634`, `fy=936.165`, `cx=659.789`, `cy=363.855` plus five distortion coefficients. Current LEGO mount is centered, about 3 in forward and 6 in high, roughly forward with a small unknown downward/tilt error. | MVP fixed mapping camera; measured intrinsics, native transport, and a 1,000-pose Pi route verified 2026-08-17. Extrinsics are intentionally approximate because the mount can shift. |
...
- total platform height, mass, and ground clearance;
- effective skid-steer width from timed straight and pivot motion;
